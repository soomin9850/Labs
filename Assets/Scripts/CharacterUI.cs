...
public class CharacterUI : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        mainCam = Camera.main;
        for(int i=0;i<GM.Characterlist.Length;i++)
        {
            Image image = GameObject.Find("List" + i.ToString()).transform.GetChild(0).GetComponent<Image>();
            image.color = new Color(1, 1, 1, 0);
            if (GM.Characterlist[i] != null)
            {
                image.color = new Color(1, 1, 1, 1);
                image.sprite = GM.Characterlist[i].prefab.GetComponent<CharacterManager>().Icon;
            }
        }
    }
    ...
    {
        GameObject OBJ = Instantiate(GM.Characterlist[info.Index].prefab, info.Building.position, Quaternion.identity);
    ...
    IEnumerator CharSpawn(int Index)
    {
        ...
        Icon.GetComponent<SpriteRenderer>().sprite = GM.Characterlist[Index].prefab.GetComponent<CharacterManager>().Icon;
        while (true)
        {
